...
export class AgentManager {
  ...
  private async _startAgentImpl(name: string, agentDir: string, config?: AgentConfig, org?: string): Promise<void> {
    ...
    if (this.agents.has(name)) {
      ...
      // (Our serialization refactor f3794c7 supersedes upstream's legacy
      // pendingRestarts queue + BUG-031 daemonJustCrashed quieting.)
    ...
    if (existsSync(agentEnvFile)) {
      ...
      // key parsing when a var is on line 1 (2026-05-16 silent
