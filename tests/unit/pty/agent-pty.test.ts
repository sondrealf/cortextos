...
describe('AgentPTY --dangerously-skip-permissions toggle', () => {
  // These exercise the CONFIG gate in isolation. The merged buildClaudeArgs ALSO
  // skips the flag when running as root (Claude Code rejects
  // --dangerously-skip-permissions as uid 0 — fork fix 8ac1ade), and this runner
  // executes as root. Simulate a non-root uid so the config dimension is tested
  // without the orthogonal root-skip masking it. In production-as-root the
  // root-skip correctly dominates and the flag is never passed regardless.
  let getuidSpy: ReturnType<typeof vi.spyOn> | undefined;
  beforeEach(() => { getuidSpy = vi.spyOn(process, 'getuid').mockReturnValue(1000); });
  afterEach(() => { getuidSpy?.mockRestore(); });
